...
const UserListTable = () => {
    ...
    const GetData = async () => {
        const dataUser = await axiosReq("Employees/Active");
    ...
    const tableBody = data.map((tableRow2) =>
        <tr key={tableRow2.id} className="border-b border-b-borderGray">



            <td className="py-4  text-sm text-left font-IRsans pl-6">{tableRow2?.user?.avatar}</td>


            <td className="py-4  text-sm text-left font-IRsans">{tableRow2?.user?.username}</td>
            <td className="py-4  text-sm text-left font-IRsans">{tableRow2?.user?.firstName+" "+tableRow2?.user?.lastName}</td>
            <td className="py-4  text-sm text-left font-IRsans">{tableRow2?.user?.mobileNumber}</td>

            <td className="py-4  text-sm text-left font-IRsans">{tableRow2?.user?.email}</td>
            <td className="py-4  text-sm text-center font-IRsans">{tableRow2?.user?.gender=="Male"?
        <img
        src={Man}
        alt="objectImg"
        className="w-[45px] h-[45px] rounded-full block mx-auto" />:
        ...
        className="w-[45px] h-[45px] rounded-full block mx-auto" />
            ...
            <td className="py-4  text-sm text-center font-IRsans"><span className="font-bold text-green">
                Yes
            </span></td>
            <td className="py-4  text-sm text-center font-IRsans pr-6"><div className="relative">
                < button onClick={() =>{setShowAction(!showAction);setID(tableRow2?.user?.userId)}}>
                    <Bullet />
                </button>
                {showAction && id==tableRow2?.user?.userId
                    ? <div className="z-50  absolute  right-[55px] top-[2px]">
                        <div className="triangleRight"></div>
                        <div
                            className="z-50 w-[135px] h-auto  bg-white shadow-whiteShodow  rounded-lg mBg">
